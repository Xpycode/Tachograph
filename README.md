...
- Accessibility permission granted to Tachograph (System Settings → Privacy & Security → Accessibility)
...
Tachograph records every key you press while capture is running. **No log data ever leaves the app** — there's no networking code, no analytics, no disk writes. Quitting the app discards the session. Clipboard export is explicit and user-initiated.
